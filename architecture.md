...
* **Service:** GitHub Actions (Scheduled Workflow) или Supabase pg_cron.
* **Frequency:** Каждый час (`0 * * * *`).
* **Logic:** Единый эндпоинт `/api/cron/hourly`.
    * Проверяет всех пользователей.
    * Вычисляет локальное время каждого пользователя (`UTC + offset`).
    * Если **09:00 Local**: Шлет Daily Digest (План на день из Google Calendar).
    * Если **00:00 Local**: Шлет напоминание заполнить журнал за прошедший день.
...
5. `POST /api/cron/hourly`
* Защищен секретным ключом.
* Вызывается каждый час.
* Умная маршрутизация уведомлений в зависимости от часового пояса пользователя.
