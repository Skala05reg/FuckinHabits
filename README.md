...
- **Команда и кнопка задач:** `/tasks`, `/today` и кнопка "Показать задачи на сегодня" отправляют актуальный список задач.
- **Антидублирование списков:** При отправке нового списка старые сообщения-списки автоматически удаляются.
...
- `PMW_ADMIN_WEBAPP_URL` — дополнительный Mini App для панели управления PussyMoneyWeed. Он показывается отдельной кнопкой в ответе на `/start` всем пользователям бота, а доступ уже внутри PMW-панели ограничивается через Telegram `initData` и `TELEGRAM_ADMIN_IDS`.
